...
public class PowerManagerService extends IPowerManager.Stub
    ...
    static final String CONTROL_WAKELOCK = "android.PowerManager.ControlWakeLocks";
    
    //static final int PARTIAL_WL_FLAG = 4;
    //static final int SCREEN_WL_FLAG = 2;
    //static final int WIFI_WL_FLAG = 1;
    ...
    private HashMap<String, Integer> mAppRuntimePermissions = null;
    private ArrayList<WakeLock> mBlockedLocks = new ArrayList<WakeLock>();
    ...
    */
    ...
    private final class ControlWakeLocksReceiver extends BroadcastReceiver {
    	
    	String action = "android.PowerManager.ControlWakeLocks";
		@Override
		public void onReceive(Context context, Intent intent) {
			if(intent.getAction().equals(action)) {			
				try {
					mAppRuntimePermissions = (HashMap<String, Integer>) intent.getSerializableExtra("AppRuntimePermissions");
	
					// check whether any running apps have permission which is revoked.
					for (RunningAppProcessInfo appProcessInfo : mActivityService.getRunningAppProcesses()) {
						Integer appPermissions = mAppRuntimePermissions.get(appProcessInfo.processName);
						if(appPermissions!= null) {
							boolean cpuLockPerm = ((appPermissions & PARTIAL_WL_MASK) == PARTIAL_WL_MASK) ? true : false; 
							boolean screenLockPerm = ((appPermissions & SCREEN_WL_MASK) == SCREEN_WL_MASK) ? true : false;
							boolean wifiLockPerm = ((appPermissions & WIFI_WL_MASK) == WIFI_WL_MASK) ? true : false;
							if (cpuLockPerm) {
								for(WakeLock wakeLock : mLocks) {
									if(wakeLock.pid == appProcessInfo.pid && 
											(wakeLock.flags & LOCK_MASK) == PowerManager.PARTIAL_WAKE_LOCK) {
										releaseWakeLock(wakeLock.binder, wakeLock.flags);
										displayToastMsg("Acquired Partial WakeLock released !");
										mBlockedLocks.add(wakeLock);
										break;
									}
								}							
							}
							else {
								WakeLock removeWakeLock = null;
								for(WakeLock wakeLock : mBlockedLocks) {
									if(wakeLock.pid == appProcessInfo.pid &&
											(wakeLock.flags & LOCK_MASK) == PowerManager.PARTIAL_WAKE_LOCK) {
										acquireWakeLock(wakeLock.flags, wakeLock.binder, wakeLock.tag, wakeLock.ws);
										displayToastMsg("Re-acquiring Partial Wakelock !");
										removeWakeLock = wakeLock;
										break;
									}
								}
								if(removeWakeLock != null) mBlockedLocks.remove(removeWakeLock);
							}
							
							if (screenLockPerm) {
								for(WakeLock wakeLock : mLocks) {
									if(wakeLock.pid == appProcessInfo.pid && 
											(wakeLock.flags & LOCK_MASK) == PowerManager.SCREEN_BRIGHT_WAKE_LOCK) {
										releaseWakeLock(wakeLock.binder, wakeLock.flags);
										displayToastMsg("Acquired Screen Wakelock released !");
										mBlockedLocks.add(wakeLock);
										break;
									}
								}							
								...
								for(WakeLock wakeLock : mBlockedLocks) {
									if(wakeLock.pid == appProcessInfo.pid &&
											(wakeLock.flags & LOCK_MASK) == PowerManager.SCREEN_BRIGHT_WAKE_LOCK) {
										acquireWakeLock(wakeLock.flags, wakeLock.binder, wakeLock.tag, wakeLock.ws);
										displayToastMsg("Re-acquiring Screen Wakelock !");
										removeWakeLock = wakeLock;
										break;
									}
								}
		        ...
		        {
		        	//Log.e("ControlWakeLock", e.getMessage());
		        	displayToastMsg("Invalid WakeLock Permission map received !");
		        	e.printStackTrace();
		        }
			}
		}
    ...
    PowerManagerService() {
        ...
        // Register with wakelock receiver
    }
    ...
    void initInThread() {
       ...
       ContentResolver resolver = mContext.getContentResolver();
        ...
        filter = new IntentFilter();
        filter.addAction(CONTROL_WAKELOCK);
        mContext.registerReceiver(new ControlWakeLocksReceiver(), filter);
    ...
    public void acquireWakeLockLocked(int flags, IBinder lock, int uid, int pid, String tag,
        ...
        // Check if the user has granted wakelock permission or not
        try {
	        //ActivityManagerService am = ActivityManagerService.self();
	        String appName =  "";
	        for (RunningAppProcessInfo appProcessInfo : mActivityService.getRunningAppProcesses()) {
				if(appProcessInfo.pid == pid) {
					appName = appProcessInfo.processName;
					break;
				}
			} 
	        if(mAppRuntimePermissions != null)
	        {
				Integer appPermissions = mAppRuntimePermissions.get(appName);
				if(appPermissions != null ) {
					boolean cpuLockPerm = ((appPermissions & PARTIAL_WL_MASK) == PARTIAL_WL_MASK) ? true : false; 
					boolean screenLockPerm = ((appPermissions & SCREEN_WL_MASK) == SCREEN_WL_MASK) ? true : false;
					boolean wifiLockPerm = ((appPermissions & WIFI_WL_MASK) == WIFI_WL_MASK) ? true : false;
		        	if(cpuLockPerm && (flags & LOCK_MASK) == PowerManager.PARTIAL_WAKE_LOCK) {
		        		displayToastMsg("Wakelock permission denied !");
		        		Log.i("ControlWakeLock", "ControlWakeLock : lock not acquired for " +appName);
		        		mBlockedLocks.add(new WakeLock(flags, lock, tag, uid, pid));
		        		return;
		        	}
		        	if(screenLockPerm && (flags & LOCK_MASK) == PowerManager.SCREEN_BRIGHT_WAKE_LOCK) {
		        		displayToastMsg("Wakelock permission denied !");
		        		Log.i("ControlWakeLock", "ControlWakeLock : lock not acquired for " +appName);
		        		mBlockedLocks.add(new WakeLock(flags, lock, tag, uid, pid));
		        		return;
        ...
        {
        	//Log.e("ControlWakeLock", e.getMessage());
        	displayToastMsg("Unable to stop wakelock acquire !");
        	e.printStackTrace();
        }
    ...
    public void displayToastMsg(final String message) 
    {
    ...
    private void releaseWakeLockLocked(IBinder lock, int flags, boolean death) {
    	...
    	// Check if the user has granted wakelock permission or not         
        if(mBlockedLocks != null)
        {
        	WakeLock removeWakeLock =  null;
        	for (WakeLock wakeLock : mBlockedLocks)
        	{
        		if(wakeLock.binder == lock)
        		{
            		displayToastMsg("Wakelock release permission denied !");
            		Log.i("ControlWakeLock", "ControlWakeLock : lock not released !");
            		removeWakeLock = wakeLock;
            		break;
        		}
        	}
			if(removeWakeLock != null) {
				mBlockedLocks.remove(removeWakeLock);
        		return;
			}
        }
